...
export const generatePresignedUrlClass = async (req, res) => {
    try {
        const { classId, files } = req.body;
        if (!classId) {
        ...
        if (files && Array.isArray(files) && files.length > 0) {
            const fileUrls = await Promise.all(files.map(async (file) => {
                const fileId = uuidv4();
                const objectKey = `class/${classId}/materials/${fileId}-${file.fileName}`;
                const command = new PutObjectCommand({
                    Bucket: process.env.S3_BUCKET_NAME,
                    Key: objectKey,
                    ContentType: file.fileType,
        ...
        if (files) {
            const fileId = uuidv4();
            const objectKey = `class/${classId}/materials/${fileId}-${files.fileName}`;
            const command = new PutObjectCommand({
                Bucket: process.env.S3_BUCKET_NAME,
                Key: objectKey,
                ContentType: files.fileType,
...
export const generateViewUrl = async (req, res) => {
    try {
        ...
        const url = await getSignedUrl(s3, command, { expiresIn: 300 }); // Valid for 5 minutes
        ...
        return res.status(500).json({ message: "Failed to generate view URL", success: false });
    }
};